...
def multi_step_amperometry_level(**kwargs):
    ...
    * trigger_at_level_lines : list --
    ...
    trigger_at_level_lines = kwargs.get('trigger_at_level_lines', [False, False, False, False])
    multi_step_amperometry_level.UseTriggerOnStart = trigger_at_level
    # pack into bitmask:
    lines = 0
    for i, high in enumerate(trigger_at_level_lines):
        if high:
            lines |= 1 << i
    multi_step_amperometry_level.TriggerValueOnStart = lines
